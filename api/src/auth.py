...
from auth_utils import listar_mutantes, listar_professores, verificar_adm
...
def decode_token(token: str, session: Session) -> Optional[User]:
    ...
    for p in listar_professores(session): # Carregando todos os professores do banco
        if getattr(p, "usuario", None) == token:
            if verificar_adm(p.usuario):
                return User(id=p.id, nome=p.nome, identificador=p.usuario, tipo="admin")
            else:
